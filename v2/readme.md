...
| Operator workspace | generic screens, dimensions, video mode, two text-position banks, text rows, deck columns, reference screen, collapsed set IDs | local storage only |
| Live runtime | selected cue, editor focus, deck history, camera preview streams, selected source, popup handles, drag state, notices | memory only |
...
“Stage” and “Audience” are only default labels. The operator can add any number of screens, rename them, remove them, and change their positive integer width/height. `previewScreenId` chooses which screen supplies deck-thumbnail geometry. The Preview profile selector inside View changes that reference only; it never routes content or changes a live output.

Older v2 workspace values are normalized in place. The former screen `video` flag plus global `fitVideo` become each screen's `off`, `cover`, or `contain` mode. Columns clamp to 2–10, text rows to 1–20, invalid positions return to their defaults, and screen/reference IDs are repaired. Collapsed IDs are deduplicated and reconciled against the current deck whenever it is hydrated, restored, replaced, reset, or pruned.
...
3. The controller always starts in Operate mode and standby. Saved words, screen profiles, view density, and each surviving set's collapsed state return; the previous live cue, streams, and popup handles do not.
...
1. Edit the deck title, set title, slide label, set attribution, and screen label directly in either mode. Reorder or remove slides and sets at any time; quiet move/remove controls reveal on hover or keyboard focus. Changes persist immediately, and Undo/Redo remain beside the primary document actions.
...
6. Drag only from the four-direction move handle. Slides move within their set; sets move within the deck. Collapse a set to reduce visual load without changing the deck or live cue; that set's state returns after reload.
...
Attribution belongs to the set and is emitted only with that set's first slide. It is edited through the native Attribution disclosure in the set header, not on each slide. On output it preserves authored line breaks as small, dim metadata flush to the lower-left edge with zero padding; it never replaces or resizes the main words layer.

### 3. Configure screens and video

1. Add and rename screens in either mode. Resize or remove a screen inside its native Advanced options disclosure. Every removal requires confirmation and remains outside deck Undo. Any number of screens is supported; labels never select special behavior.
...
4. Open Advanced options to set text placement with the 3×3 grid. Off edits the Words-only layout bank; Cover or Contain edits the Video layout bank. Each screen remembers both.
5. In the same disclosure, set the desired text-row capacity from 1 to 20. Eight is the default. A summary warning reports when current copy overflows while the controls stay collapsed.
6. Use Preview profile inside View only to choose which screen profile shapes deck thumbnails. It does not route content or choose a destination.
...
6. Watch the amber slide border and its LIVE caption for words state. If its set is collapsed, the set header retains a compact LIVE and slide-number cue. The amber source border shows source state, and each screen preview shows the final screen-local result.
...
- Popup-blocked, lost-output, import, and source failures appear in their own panel rather than changing unrelated state.
...
- Add set and the compact set-header Add slide action create records and focus the new words editor without cueing unfinished text.
...
- Clicking a set header collapses it without changing the deck, live cue, or popup output; a collapsed live set keeps visible LIVE identification, and stable set IDs keep each set's saved state independent.
- One native four-direction move handle and remove control remain available per slide/set. Only the handle is draggable, so text selection never starts a drag.
...
- Screen position, rows, dimensions, and removal live in each native Advanced options disclosure.
...
The architecture is simpler than v1, and the visual-density pass resolves the persistent cue/count readouts, detached view controls, set attribution distance, screen-control borders, and one-dimensional move affordances. The remaining rows are current behaviors, not hidden implementation notes:
...
The pinned top bar has exactly two compact bands. The first holds the editable deck title opposite Undo, Redo, Add set, and Deck; the second holds the native Edit words switch opposite View. The live cue, slide count, and column count are not repeated in the header. View contains Preview profile and a Large-to-Small thumbnail slider, while the slide grid consumes the complete deck width.
...
The nine-way position grid and row slider remain physically attached to the composition they change, but are hidden in the native Advanced options disclosure until needed. Arrow glyphs express the eight positions and a dot expresses center.
...
There is exactly one selected source, but the source strip intentionally keeps every available camera preview visible. On controller startup, Presently requests modest 640×360, 15 fps streams for each enumerated camera. Selecting a camera promotes that same track in place toward 1920×1080 at 30 fps; deselecting or switching returns the previous track to preview quality. This keeps one capture per physical camera and gives every output program-quality pixels without reopening the device.

Quality changes are serialized per track and reconcile to the newest selection, so rapid switching cannot leave a stale camera promoted. A failed high-quality constraint keeps the selected preview stream live and reports the fallback. `Refresh` acquires a complete replacement inventory before stopping the old one when possible, preserves selection by stable device ID, promotes the replacement selected camera, and stops every superseded track. A small hide control stops one preview until the next refresh.

Clicking a camera card selects it for outputs; clicking the selected card returns outputs to no source without destroying the preview inventory. Shared display is represented by the same kind of card and is not camera-quality constrained. Cancelling or failing display sharing preserves the current source. Requests are ordered independently for camera inventory and source choice. Every resolved stream is immediately pending, adopted into the inventory, or stopped. If a slow refresh/share resolves after newer intent—or after root disposal—its tracks are stopped and it cannot replace current state.
